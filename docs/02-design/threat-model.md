...
| **T7** | **Misconfiguración cloud** / transferencia transfronteriza insegura (A02) | 8 | 4 | 4 | 8 | 5 | **5.8** | RS-11; endurecer IAM/redes/buckets; mecanismo legal de transferencia (`<TODO>`) |
...
- **T4, T7** → Modelo A ([ADR-0003](../00-project/adr/0003-hosting-modelo-a.md)) + cifrado + control de acceso (RS-02/RS-03); hardening cloud (RS-11).
- **T5** → NeMo Guardrails ([ADR-0002](../00-project/adr/0002-nemo-guardrails-prompt-injection.md)) + LLM no autoritativo (RS-13).
- **T6** → acreditación + MFA + handshake (RS-01).
- **T9** → proof-of-life como link por login (RF-09, RS-08).
- **T10** → merge reversible (RF-11).
- **T11** → logging inmutable (RS-06).
- **T12** → supply chain del LLM on-prem ([ADR-0001](../00-project/adr/0001-llm-on-premises.md)) y deps (RS-12).

## Decisiones abiertas

- `<TODO>` Mecanismo legal de transferencia transfronteriza (T7) — ver ADR-0003.

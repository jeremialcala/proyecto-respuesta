"""Consume outbound.reply y delega en el OutputService (ADR-0005/0012). `boto3` perezoso."""
...
class SqsConsumer:
    def __init__(self, queue_url: str, region: str, service: OutputService,
    ...
    def start(self) -> None:
        client = self._ensure()
        self._running = True
        log.info("escuchando outbound.reply en %s", self._queue_url)
        while self._running:
